...
- **Prête-noms en repli du quota titulaire, plafond configurable (règle 2026-07-26, ADR-016)** : chaque `BookingRule` a un `maxDailyReservationsPerPlayer` (défaut 2, passé au moteur local — limite de courtoisie propre au groupe, pas une contrainte TeamR, peut différer d'un groupe à l'autre) et une liste ordonnée `substituteBookers` (prête-noms, userIds resa-squash).
