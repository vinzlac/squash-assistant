...
- **Escalade automatique min→max joueurs/court, alerte de capacité, fenêtre de disponibilité (règle 2026-07-22, ADR-014)** : avant de figer le plan d'une heure candidate, squash-assistant vérifie que la capacité des courts suffit pour tous les joueurs confirmés.
  ...
  - **Libellé volontairement neutre (2026-07-25)** : ne dit plus "capacité des courts insuffisante" — le manque peut aussi venir d'un garde-fou côté resa-squash (ex. plafond de résas/jour vérifié uniquement sur le titulaire de la clé API, cf. `plan.warnings`) plutôt que d'un vrai manque de courts disponibles. Constaté en usage réel : un plan s'est arrêté avec un court effectivement libre sur TeamR, la vraie cause étant `"Titulaire clé API : plafond 2 résas ce jour atteint — arrêt du plan."` — le détail complet des `warnings` resa-squash est maintenant affiché à l'étape 3 (UI) sous chaque heure en shortfall, et le message WhatsApp final (étape 4) ne prétend plus non plus une cause précise ("n'ont pas pu être réservé(s) cette semaine", sans mention de "capacité des courts").
...
- **Prête-noms en repli du quota titulaire, plafond configurable (règle 2026-07-26, ADR-016)** : chaque `BookingRule` a un `maxDailyReservationsPerPlayer` (défaut 2, transmis à `plan_group_bookings` — limite de courtoisie propre au groupe, pas une contrainte TeamR, peut différer d'un groupe à l'autre) et une liste ordonnée `substituteBookers` (prête-noms, userIds resa-squash).
  ...
  - Pour chaque heure, les `substituteBookers` non déjà utilisés (`usedTodayIds` + confirmés de cette heure) sont transmis à `plan_group_bookings` (`substitutePlayerIds`) : resa-squash les consomme si besoin (effectif impair, **ou désormais titulaire de la clé API à quota**, voir [ADR-010 resa-squash](../../../resa-squash/docs/adr/010-quota-titulaire-scope-et-prete-noms-substitution.md), repo séparé) — squash-assistant ne décide pas *quand* substituer, seulement *qui est éligible*.
  ...
  - Corrige au passage un bug constaté en usage réel : le plafond de résas/jour ne bloque plus tout le plan dès que le titulaire de la clé API l'atteint, seulement les paires qui l'impliquent réellement (voir ADR-010 resa-squash pour le détail du fix côté MCP).
