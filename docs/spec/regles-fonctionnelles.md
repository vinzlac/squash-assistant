...
- **Description générée en français (2026-07-22)** : `describeRuleInFrench` (`packages/db/src/ruleDescription.ts`, testé indépendamment) produit une description exhaustive et déterministe (aucun appel LLM) de tous les paramètres d'une règle — affichée dans un `<details>` repliable sur la page d'édition. Sert aussi de fixture de référence pour les futurs tests d'intégration LLM (description → paramètres).
