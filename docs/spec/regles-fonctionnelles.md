...
- **Prête-noms en repli du quota titulaire, plafond configurable (règle 2026-07-26, ADR-016)** : chaque `BookingRule` a un `maxDailyReservationsPerPlayer` (défaut 2, transmis à `plan_group_bookings` — limite de courtoisie propre au groupe, pas une contrainte TeamR, peut différer d'un groupe à l'autre) et une liste ordonnée `substituteBookers` (prête-noms, userIds resa-squash).
  ...
  - Pour chaque heure, les `substituteBookers` non déjà utilisés (`usedTodayIds` + confirmés de cette heure) sont passés au moteur local : squash-assistant (via `apps/worker/src/planning/groupBookingPlan.ts`) les utilise si besoin (effectif impair, **ou titulaire de la clé API à quota**, voir [ADR-010 resa-squash](../../../resa-squash/docs/adr/010-quota-titulaire-scope-et-prete-noms-substitution.md), repo séparé, pour la justification du comportement original) — squash-assistant ne décide pas *quand* substituer, seulement *qui est éligible*.
