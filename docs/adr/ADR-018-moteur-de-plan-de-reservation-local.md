...
- Limitation héritée du moteur original, **corrigée le 2026-08-01** (post-port, hors périmètre initial "port fidèle") : quand le titulaire de la clé API est substitué par un prête-nom faute de quota sur un round donné, la continuité de court de cette paire sur un round **ultérieur** était recalculée par rapport à l'identité d'origine de la paire (`userId`/`partnerId` de la réservation TeamR), pas par rapport à l'identité réelle du joueur qui joue — la continuité pouvait donc silencieusement se rompre, y compris quand le prête-nom change d'un round à l'autre (round 1 substitué par un prête-nom, round 2 par un autre). Corrigé en ajoutant `pairUserId`/`pairPartnerId` à `ProposedSlot` (`courtAssignment.ts`) : l'identité réelle de la paire (avant toute substitution) est portée en plus de l'identité de réservation, et `previousCourtForPair` compare sur cette identité réelle. Ce n'était pas une régression introduite par le port ; c'était un comportement déjà présent dans l'algorithme resa-squash d'origine.
