...
- **Complément 2026-09-01 ([ADR-024](./ADR-024-joker-reservation-joueur-refuse.md))** : un **joker** (`BookingRule.jokerBookerId`) traite le cas voisin mais distinct d'un joueur que **TeamR refuse** au moment du `reserve_slot` — pas réinscrit pour la saison, ou quota atteint. Règles de consommation opposées à celles d'un prête-nom : le joker n'est pas consommé pour la journée (c'est le gérant du club, sans plafond), mais il reste limité à une réservation par créneau horaire. Les deux mécanismes coexistent sans se remplacer.
